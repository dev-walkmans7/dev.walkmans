...
const userRepository = {
  fineOneWithRole: async (params) => {
    try {
      let user = await User.findOne({
        $or: [{ email: params.username }, { phone: params.username }],
        isDeleted: false,
        isActive: true,
      })
        .populate("role")
  ...
  getAllUsers: async (req) => {
    try {
      ...
      if (_.isObject(req.body.query) && _.has(req.body.query, "Status")) {
        ...
        !_.isEmpty(req.body.query_status) &&
        !_.isUndefined(req.body.query_status) &&
        _.has(req.body, "query_status")
      ) {
        and_clauses.push({
          isActive: req.body.query_status == "active" ? true : false,
        });
      }
  ...
  getUserDetailById: async (id) => {
    try {
      let user = await User.findOne(
        { _id: id },
        { first_name: 1, last_name: 1, email: 1, phone: 1, user_type: 1 }
      ).exec();
